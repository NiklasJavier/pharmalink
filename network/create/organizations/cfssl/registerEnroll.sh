...
function peer_cert() {
    ...
    mkdir -p "organizations/peerOrganizations/data/ca"
    mkdir -p "organizations/peerOrganizations/data/msp/cacerts"
    mkdir -p "organizations/peerOrganizations/data/msp/tlscacerts"
    mkdir -p "organizations/peerOrganizations/data/peers"
    mkdir -p "organizations/peerOrganizations/data/tlsca"

    CERT_DIR=organizations/peerOrganizations/data
    ...
    if [ ! -f "$CERT_DIR/ca/ca-key.pem" ]; then
        ...
        cp "$CERT_DIR/ca/ca.pem" "$CERT_DIR/ca/ca-cert.pem"
...
function orderer_cert() {
    ...
    mkdir -p organizations/ordererOrganizations/data/ca
    mkdir -p organizations/ordererOrganizations/data/msp/cacerts
    mkdir -p organizations/ordererOrganizations/data/msp/tlscacerts
    mkdir -p organizations/ordererOrganizations/data/orderers
    mkdir -p organizations/ordererOrganizations/data/tlsca

    CERT_DIR=organizations/ordererOrganizations/data

    if [ ! -f "$CERT_DIR/ca/ca-key.pem" ]; then

        cfssl gencert -initca "${PWD}/organizations/cfssl/ca-orderer.json" | cfssljson -bare "$CERT_DIR/ca/ca"

        cp "$CERT_DIR/ca/ca.pem" "$CERT_DIR/tlsca/tlsca.data-cert.pem"

        cp "$CERT_DIR/ca/ca.pem" "$CERT_DIR/msp/cacerts/"
        cp "$CERT_DIR/ca/ca.pem" "$CERT_DIR/msp/tlscacerts/tlsca.data-cert.pem"
...
function generate_orderer_certs() {
    ...
    for DIR in cacerts keystore signcerts tlscacerts; do
        mkdir -p "organizations/ordererOrganizations/data/orderers/$USER/msp/$DIR"
    done

    mkdir -p "organizations/ordererOrganizations/data/orderers/$USER/tls"
